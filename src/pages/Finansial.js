...
const Finansial = () => {
  ...
  const [sortFinansial, setSortFinansial] = useState("ASC");
  ...
  // Reset currentPage to 1 when filteredFinansial changes
  useEffect(() => {
    setCurrentPage(1);
  }, [filteredFinansial]);
  ...
  const dataToDisplay = filteredFinansial.length > 0 ? filteredFinansial : allFinansial;
  ...
  const handleSortChange = async (e) => {
    setSortFinansial(e.target.value);
  };
  ...
  return (
    <div className="d-flex data-finansial">
      ...
      <div className="p-2 table-data-finansial">
        <h1>Finansial</h1>
        <SearchBar placeholder="Search finansial by name..." />
              ...
              {currentData.length > 0 ? (
                currentData.map((item, index) => (
                  <tr key={item.id || index}>
                    ...
                    <td>{formatCurrency(item.modal_awal)}</td>
